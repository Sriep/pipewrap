...
void WorkflowTreeView::executeSlot()
{
    ...
    while(item)
    {
        ...
        do
        {
            ...
            if (child)
            {           
                if (child->data(UsageRole) == OutPipe)
                {
                    pipe_out = true;
                }
                else if (child->data(UsageRole) == InPipe)
                {
...
void WorkflowTreeView::varyOpSlot()
{
    ...
    if (item->data(DataTypeRole).toString() == "int")
    {
        ...
        if (dialog.exec())
        {
            ...
            from_command->setData(StructureUsageRole, CommandUsageRole);
...
void WorkflowTreeView::pipe(QModelIndex out, QModelIndex in)
{
    ...
    out_pipe_label->setData(OutPipe,UsageRole);
    out_pipe->setData(OutPipe,UsageRole);
    ...
    in_pipe_lable->setData(InPipe,UsageRole);
    in_pipe->setData(InPipe,UsageRole);
