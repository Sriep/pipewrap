...
{/*
    ...
    pipeStatus->moveToThread(&pipeStatusThread);

    connect(process, SIGNAL(readyReadStandardError()),
            this, SLOT(processStdErrSlot()));
    connect(this, SIGNAL(processStdErr(const QString& message)),
            pipeStatus, SLOT(stdErrOutSlot(const QString& message)));

    connect(process,
            SIGNAL(finished(int exitCode, QProcess::ExitStatus exitStatus)),
            pipeStatus, SLOT(processFinishedSlot(int exitCode,
                                     QProcess::ExitStatus exitStatus)));
    connect(process, SIGNAL(error(QProcess::ProcessError error)),
            pipeStatus, SLOT(processErrorSlot(QProcess::ProcessError error)));

    connect(&pipeStatusThread, &QThread::finished, pipeStatus, &QObject::deleteLater);
    connect(this, &WorkflowTreeView::startPipeStatus, pipeStatus, &PipeStatus::show);

    //connect(pipeStatus, &pipeStatus::destroyed, this, &WorkflowTreeView::handleResults);
    //pipeStatus->show();
